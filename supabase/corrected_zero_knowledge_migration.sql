...
-- Add encryption version column to messages table
...
-- Create view for secure messages (version 3+)
...
-- Create audit table for security monitoring (FIXED VERSION)
CREATE TABLE IF NOT EXISTS encryption_audit (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID NOT NULL,
  action TEXT NOT NULL, -- actions: key_generated, key_published, message_encrypted, migration_completed
  details JSON, -- Changed from JSONB to JSON for compatibility
  created_at TIMESTAMPTZ DEFAULT NOW() NOT NULL
);
...
-- Create function to log encryption events (FIXED VERSION)
CREATE OR REPLACE FUNCTION log_encryption_event(
  p_user_id UUID,
  p_action TEXT,
  p_details JSON DEFAULT NULL -- Changed from JSONB to JSON for compatibility
) RETURNS VOID AS $$
BEGIN
...
-- Insert initial migration marker (FIXED VERSION)
INSERT INTO encryption_audit (user_id, action, details)
VALUES (
  '00000000-0000-0000-0000-000000000000'::UUID,
  'zero_knowledge_migration_applied',
  '{"migration_version": "20250826", "description": "Added zero-knowledge encryption support"}'::JSON -- Changed from JSONB to JSON
) ON CONFLICT DO NOTHING;

-- Add helpful comments
COMMENT ON TABLE secure_messages IS 'View showing only messages encrypted with zero-knowledge encryption (version 3+)';
COMMENT ON COLUMN messages.encryption_version IS 'Encryption version: 1=SharedSecret (COMPROMISED), 2=FastXOR (COMPROMISED), 3=ZeroKnowledge (SECURE)';
...
BEGIN
  ...
  RAISE NOTICE 'Zero-knowledge encryption migration completed successfully';
END $$;