...
exports.SecureE2EVideoStorageZeroKnowledge = void 0;
const FileSystem = __importStar(require("expo-file-system"));
const next_1 = require("expo-file-system/next");
const supabase_1 = require("../services/supabase");
const react_native_1 = require("react-native");
const videoCompression_1 = require("../services/videoCompression");
...
const friendEncryption_1 = __importDefault(require("./friendEncryption"));
// React Native performance debugging flags
const ENABLE_PERFORMANCE_LOGGING = true;
class SecureE2EVideoStorageZeroKnowledge {
    /**
     ...
     */
    static async encryptAndUploadVideo(videoUri, senderId, recipientId, onProgress, onCompressionProgress) {
        const startTime = Date.now();
        let compressedVideoUri = null;
        try {
            ...
            const compressionResult = await (0, videoCompression_1.compressVideo)(videoUri, (compProgress) => {
            ...
            const recipientPublicKey = await SecureDeviceKeys_1.default.getLatestPublicKeyForUser(recipientId);
            ...
            try {
                ...
                const compressedFile = new next_1.File(compressedVideoUri);
            ...
            const encryptionResult = await NaClBoxEncryption_1.default.encryptBinary(videoBase64, recipientPublicKey);
            ...
            if (compressedVideoUri) {
                await (0, videoCompression_1.cleanupCompressedVideo)(compressedVideoUri);
            }
            ...
            // Cleanup on error
            if (compressedVideoUri) {
                await (0, videoCompression_1.cleanupCompressedVideo)(compressedVideoUri).catch(console.error);
            }
     ...
     */
    static async downloadAndDecryptVideo(videoId, encryptedKey, keyNonce, dataNonce, ephemeralPublicKey, myUserId, onProgress) {
        const startTime = Date.now();
        try {
            console.log(`[ZK Video Download] Starting zero-knowledge video download for videoId: ${videoId}`);
            onProgress?.(0.1);
            // Get device keys for decryption
            const deviceKeys = await friendEncryption_1.default.getDeviceKeys();
            if (!deviceKeys) {
            ...
            const { data: encryptedData, error } = await supabase_1.supabase.storage
                .from('videos')
            ...
            const decryptedData = await NaClBoxEncryption_1.default.decryptBinary(encryptedBase64, encryptedKey, keyNonce, dataNonce, ephemeralPublicKey, deviceKeys.privateKey);
            ...
            if (react_native_1.Platform.OS === 'ios' && !outputPath.startsWith('file://')) {
     ...
     */
    static async downloadAndDecryptVideoLegacy(videoId, encryptedKey, keyNonce, senderId, recipientId, onProgress, videoNonce) {
...
exports.SecureE2EVideoStorageZeroKnowledge = SecureE2EVideoStorageZeroKnowledge;
